    ...
    const handleMessage = useCallback((data) => {
        ...
        if (data.sender._id === 'ai') {
            try {
                ...
                webContainer?.mount(messageData.fileTree);
                setFileTree(messageData.fileTree || {});
                } else if (messageData.code?.file?.name && messageData.code?.file?.contents) {
                // Fallback support if Gemini uses incorrect "code" format
                const { name, contents } = messageData.code.file;
                const fallbackTree = {
                    [name]: {
                    file: { contents }
                    }
                };
                webContainer?.mount(fallbackTree);
